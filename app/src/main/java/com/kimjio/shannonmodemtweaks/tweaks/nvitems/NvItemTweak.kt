...
import com.kimjio.shannonmodemtweaks.tweaks.TweakType
import com.kimjio.shannonmodemtweaks.utils.ExecuteAsRoot
...
abstract class NvItemTweak : Tweak() {
    ...
    private fun parseGetNvItemOutput(output: String): List<NvItem> {
        ...
        for (l in lines) {
            ...
            val nvItem = "\"(.+)\",(\\d),\"(.+)\"".toRegex()
                .find(line.removePrefix("+GOOGGETNV: "))?.groupValues

            if (nvItem == null || nvItem.size != 4) {
                continue
            }

            val id = nvItem[1]
            val index = nvItem[2].toInt()
            val value = nvItem[3]
    ...
    override suspend fun isTweakEnabled(isReload: Boolean, onProgress: (Int, Int) -> Unit): Boolean {
        ...
        if (isEnabled != null) {
            onProgress(nvItems.size, nvItems.size)
            return isEnabled!!
        }
        ...
        isEnabled =
            nvItems.all {
                ...
                onProgress(setNvItems.size, nvItems.size)
        ...
        if (setNvItems.size != nvItems.size) {
            onProgress(nvItems.size, nvItems.size)
        }
    ...
    private fun applyNvItemChanges(): Pair<Boolean, String> {
        ...
        Timber.d("Executing command: $cmd")
        val result =
            ExecuteAsRoot.executeAsRoot(listOf(cmd)) ?: return Pair(
                false,
